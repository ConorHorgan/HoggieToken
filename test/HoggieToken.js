...
contract('HoggieToken', function(accounts) {
	...
	it('initializes the contract with the correct values', function() {
		return HoggieToken.deployed().then(function(instance) {
			...
			assert.equal(name, 'Hoggie Coin', 'has the correct name');
			return tokenInstance.symbol();
		}).then(function(symbol) {
			assert.equal(symbol, 'HOGGIE', 'has the correct symbol');
			return tokenInstance.standard();
		}).then(function(standard) {
			assert.equal(standard, 'Hoggie Coin v1.0', 'has the correct standard');
		})
	})
	...
	it('transfers token ownership', function() {
		return HoggieToken.deployed().then(function(instance) {
			...
			assert.equal(receipt.logs[0].args._to, accounts[1], 'logs the account the tokens are transfered toNumber');
			...
			assert.equal(balance.toNumber(), 750000, 'deducts to amount from the sending account');
		})
	});
});